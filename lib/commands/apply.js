    ...
    .action(async (options) => {
      ...
      try {
        ...
        if (!activeRackName) {
          ...
          console.log('\n' + `Use ${chalk.cyan('rax use <rackname>')} to set an active rack.`);
          return;
        }
        ...
        if (!rackConfig.servers || rackConfig.servers.length === 0) {
          ...
          console.log('\n' + chalk.yellow('This will result in an empty server configuration for Claude Desktop.'));
          ...
          if (!options.force && !options.yes) {
            ...
            if (!response.proceed) {
              console.log('\n' + chalk.blue('Operation cancelled.'));
              return;
            }
        ...
        if (fs.existsSync(CLAUDE_CONFIG_FILE)) {
          ...
          if (backupFile) {
            ...
            console.log('\n' + chalk.magenta(`Backup saved to: ${backupFile}`));
        ...
        console.log('\n' + chalk.magenta(`Configuration file: ${CLAUDE_CONFIG_FILE}`));
        ...
        if (options.restart) {
          console.log('\n' + chalk.blue('Restarting Claude Desktop...'));
          ...
          console.log('\n' + chalk.blue('You may need to restart Claude Desktop for changes to take effect.'));
        }
      } catch (error) {
