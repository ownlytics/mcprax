  ...
  program
    ...
    .action((rackname) => {
      ...
      try {
        ...
        console.log('\n' + chalk.green(`Created rack: ${rackname}`));
      } catch (error) {
  ...
  program
    ...
    .action((rackname) => {
      ...
      try {
        ...
        console.log('\n' + chalk.green(`Activated rack: ${rackname}`));
      } catch (error) {
      ...
      if (type === 'racks') {
        ...
        try {
          ...
          if (rackFiles.length === 0) {
            console.log('\n' + chalk.yellow('No racks available. Create one with "rax create <rackname>".'));
            return;
          }
          
          console.log('\n' + chalk.blue('Available racks:'));
        ...
        console.log('\n' + chalk.yellow('Use "rax mounted" to list servers in the active rack.'));
        console.log(chalk.yellow('Use "rax server list" to list all available servers.'));
      } else {
        console.log('\n' + chalk.red(`Unknown list type: ${type}`));
      }
    });
  ...
  program
    ...
    .action(() => {
      try {
        ...
        if (!hasActiveRack()) {
          console.log('\n' + chalk.yellow('No active rack set.'));
          return;
        }
        ...
        if (!fs.existsSync(rackFile)) {
          console.log('\n' + chalk.yellow(`Active rack '${activeRackName}' does not exist.`));
          return;
        }
        ...
        console.log('\n' + chalk.green(`Active rack: ${activeRackName} (${serverCount} servers)${description}`));
        ...
        if (serverCount > 0) {
          ...
          console.log('\n' + chalk.yellow('No servers mounted in this rack.'));
