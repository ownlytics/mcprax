...
function commandRouter(program) {
  ...
  program
    ...
    .action((rackname) => {
      ...
      try {
        const activeConfig = { activeRack: rackname };
...
deleteCommand(program);
  ...
  program
    ...
    .action((type = 'racks') => {
      if (type === 'racks') {
        ...
        let activeRackName = null;
        try {
          if (fs.existsSync(ACTIVE_CONFIG)) {
            const activeConfig = fs.readJSONSync(ACTIVE_CONFIG);
            activeRackName = activeConfig.activeRack;
          }
        } catch (error) {
          console.warn(chalk.yellow(`Warning: Could not read active configuration: ${error.message}`));
        }
  ...
  program
    .command('current')
    .description('Show the currently active rack')
    .action(() => {
      try {
        if (!fs.existsSync(ACTIVE_CONFIG)) {
        ...
        if (!fs.existsSync(rackFile)) {
          console.log(chalk.yellow(`Active rack '${activeConfig.activeRack}' does not exist.`));
          return;
        }
        ...
        console.log(chalk.green(`Active rack: ${activeConfig.activeRack} (${serverCount} servers)${description}`));
      } catch (error) {
