    ...
    .action((servername) => {
      ...
      try {
        ...
        if (rackConfig.servers.includes(servername)) {
          console.log(chalk.yellow(`Server '${servername}' is already mounted in rack '${activeRackName}'.`));
          return;
        }
        ...
        console.log(chalk.green(`Mounted server '${servername}' to rack '${activeRackName}'.`));
      } catch (error) {
