    ...
    .action(async (servername, options) => {
      try {
        ...
        if (mountedRacks.length > 0) {
          console.log('\n' + chalk.yellow(`Warning: Server '${servername}' is mounted in the following racks:`));
        ...
        if (mountedRacks.length > 0) {
          for (const rackName of mountedRacks) {
            ...
            console.log('\n' + chalk.blue(`Removed server from rack: ${rackName}`));
          }
        }
        ...
        console.log('\n' + chalk.green(`Deleted server: ${servername}`));
      } catch (error) {
