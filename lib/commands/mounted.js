...
const { RACKS_DIR, SERVERS_DIR, ACTIVE_CONFIG } = require('../config');
    ...
    .action((options) => {
      try {
        ...
        if (!rackConfig.servers || rackConfig.servers.length === 0) {
          console.log(chalk.yellow(`Rack '${activeConfig.activeRack}' has no servers mounted.`));
          console.log(chalk.blue(`Use 'rax mount <servername>' to add servers to this rack.`));
          return;
        }
        
        // List servers in the rack
        console.log(chalk.blue(`Servers mounted in rack '${activeConfig.activeRack}':`));
